...
Storybook supports different JS frameworks, each JS feameowrk has its own Storybook "version".

I've created 2 separate branches to compare the implementations.
...
Please note for this demo, the usage of a JS framework is to provide an alternative template system to build the components only, it doesn't affect any functional logic which is defined by existing jQuery.
...
Please note I'm only using the web hosting / front-end functionality of Amplify for create a [simple static website deployment](https://aws.amazon.com/amplify/hosting/) pipeline, there's nothing to do with back-end functionality. 
